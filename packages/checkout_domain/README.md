...
Desde então o package não mudou, e isso é informação e não omissão. As Aulas 50 a 53 reestruturaram a apresentação do aplicativo e ampliaram a costura de autenticação sem tocar em uma linha daqui: `CheckoutSession` deixou de ser exposta à interface — o `CheckoutCubit` passou a emitir um `CheckoutViewState` do lado do app —, e nenhuma pergunta de apresentação subiu para o domínio. Um `session.canConfirmPayment` aqui faria este package saber que existe um botão. Ver [ADR 0002](../../docs/adr/0002-presentation-layer-boundary.md).
