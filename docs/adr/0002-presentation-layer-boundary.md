...
**Um limite conhecido permanece.** `canScan` é constante `true`, reproduzindo o comportamento anterior: o botão de leitura sempre esteve habilitado. É uma falha latente — `MedicationScanned` só tem transição a partir de `collectingMedication`, e apertá-lo em `checkingEligibility` lança `InvalidCheckoutTransitionException`. Corrigir muda comportamento e ficou registrado como decisão própria, com comentário no ponto exato do código.
