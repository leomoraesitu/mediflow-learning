...
O fluxo permanece local e sintético. Os repositórios demonstrativos permitem compor e exercitar `CheckoutCubit` sem infraestrutura externa, enquanto o snapshot e as duas implementações de armazenamento comprovam a fronteira serializável e substituível. `CheckoutCubit` já sabe restaurar e persistir seu estado quando recebe um `CheckoutSessionStorage`, mas o adapter Drift ainda não participa da composição real do fluxo: o app não injeta esse storage ao abrir o Modo Farmácia, então uma sessão em andamento ainda não sobrevive a um reinício fora dos testes. Também não há comunicação HTTP, cache, migrações de esquema além da versão inicial, timeouts reais, pagamento ou integrações externas, que serão introduzidos em aulas posteriores.
...
O resultado esperado é formatação limpa, análise estática sem problemas, 52 testes aprovados — incluindo acessibilidade, navegação, validação de entrada, Cubits, integração da sessão com a interface, efeito reativo de confirmação, progresso selecionado, feedback de falhas, feedback acessível de checkout concluído, submissão da receita, ações de avanço do checkout, o round-trip JSON do snapshot, o armazenamento assíncrono em memória, as operações do banco Drift, o adapter SQLite e a restauração/persistência de sessão pelo `CheckoutCubit` — e somente alterações intencionais exibidas pelo Git. O Quality Gate completo do monorepo também executa os testes de fronteira do package `checkout_domain`.
