...
`OutboxCheckoutRepository` também está conectado: `_openPharmacyMode` decora `DemoCheckoutRepository` com ele (`OutboxCheckoutRepository(inner: DemoCheckoutRepository(), database: database)`), então toda criação de checkout no app real já passa pelo ciclo de enfileirar/remover do outbox. Não existe ainda uma rotina de reenvio que percorra `readPendingOutboxEvents` na inicialização do app e tente novamente cada evento pendente usando a mesma `idempotencyKey`; o decorator garante o registro e a remoção ao redor de uma chamada, mas o "reenviar" de fato fica para uma próxima etapa.
...
Essa infraestrutura HTTP está implementada e testada isoladamente, contra um `HttpClientAdapter` fake (`FakeHttpClientAdapter`, compartilhado entre os três arquivos de teste), sem tocar rede real — e, diferente do Drift e do outbox, **continua fora da composição do aplicativo** de propósito: `_openPharmacyMode` usa `DemoPrescriptionRepository`, `DemoMedicationRepository` e `DemoCheckoutRepository` (este último decorado pelo outbox), não os repositórios Dio. Sem um backend real (Firebase Functions só chega na Aula 33), conectar `CheckoutApiClient` a um `baseUrl` fictício faria toda tentativa de checkout falhar de verdade no app. Não há ainda retry automático por tipo de falha; a classificação existe, mas quem a consome hoje são só os testes.
...
O fluxo em execução no app permanece sintético, mas a persistência que o sustenta não é mais só teórica. `BenefitsHomePage` compõe `CheckoutCubit.restore` com `DriftCheckoutSessionStorage` (SQLite real, via `CheckoutDatabase.defaults()`) e `OutboxCheckoutRepository` real — a sessão sobrevive a sair e reabrir o Modo Farmácia, e toda criação de checkout já passa pelo ciclo de outbox. O que continua fake é a validação de receita, a checagem de elegibilidade e a criação/consulta de checkout em si (`DemoPrescriptionRepository`, `DemoMedicationRepository`, `DemoCheckoutRepository` como `inner` do outbox) — os três repositórios HTTP com Dio (`DioPrescriptionRepository`, `DioMedicationRepository`, `DioCheckoutRepository`), com classificação de falhas por `NetworkFailure` e chave de idempotência, existem e estão testados isoladamente, mas não substituem os fakes na composição, porque não há backend real para eles conversarem (Firebase Functions só chega na Aula 33). Também não há cache, retry automático por tipo de falha, nem uma rotina que efetivamente reenvie os eventos do outbox — isso e a troca dos repositórios fake pelos reais serão introduzidos em aulas posteriores.
