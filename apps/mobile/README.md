...
`OutboxCheckoutRepository` também está implementado e testado isoladamente, mas nada o instancia fora dos testes — nem `main.dart`, que continua com `DemoCheckoutRepository`. Também não existe ainda uma rotina de reenvio que percorra `readPendingOutboxEvents` na inicialização do app e tente novamente cada evento pendente usando a mesma `idempotencyKey`; o decorator garante o registro e a remoção, mas o "reenviar" de fato fica para uma próxima etapa.
...
O fluxo em execução no app permanece local e sintético. Os repositórios demonstrativos (`Demo*`) continuam sendo o que `BenefitsHomePage` de fato compõe, permitindo exercitar `CheckoutCubit` sem infraestrutura externa. Três peças de infraestrutura real já existem e estão testadas isoladamente, mas nenhuma delas participa da composição do app ainda: a persistência com Drift (`DriftCheckoutSessionStorage` + `CheckoutCubit.restore`), os três repositórios HTTP com Dio (`DioPrescriptionRepository`, `DioMedicationRepository`, `DioCheckoutRepository`) com classificação de falhas por `NetworkFailure` e chave de idempotência, e o outbox local (`OutboxEvents` + `OutboxCheckoutRepository`). Não há cache, retry automático por tipo de falha, uma rotina que efetivamente reenvie os eventos do outbox, pagamento real ou integrações externas — isso e a conexão dessas três peças ao app real serão introduzidos em aulas posteriores.
