...
Essa infraestrutura HTTP está implementada e testada isoladamente, contra um `HttpClientAdapter` fake (`FakeHttpClientAdapter`, compartilhado entre os três arquivos de teste), sem tocar rede real — e, diferente do Drift e do outbox, **continua fora da composição do aplicativo**: `_openPharmacyMode` usa `DemoPrescriptionRepository`, `DemoMedicationRepository` e `DemoCheckoutRepository` (este último decorado pelo outbox), não os repositórios Dio. Desde a Aula 33 existe um backend real para eles conversarem (as Cloud Functions em `functions/`, documentadas no README daquele diretório), mas a troca dos fakes pelos repositórios Dio na composição ainda não foi feita — é o próximo passo. Não há ainda retry automático por tipo de falha; a classificação existe, mas quem a consome hoje são só os testes.
...
O fluxo em execução no app permanece sintético, mas a persistência que o sustenta não é mais só teórica. `BenefitsHomePage` compõe `CheckoutCubit.restore` com `DriftCheckoutSessionStorage` (SQLite real, via `CheckoutDatabase.defaults()`) e `OutboxCheckoutRepository` real — a sessão sobrevive a sair e reabrir o Modo Farmácia, toda criação de checkout passa pelo ciclo de outbox, e `main()` reenvia eventos pendentes via `OutboxSynchronizer` antes mesmo do primeiro frame do app. O que continua fake é a validação de receita, a checagem de elegibilidade e a criação/consulta de checkout em si (`DemoPrescriptionRepository`, `DemoMedicationRepository`, `DemoCheckoutRepository` como `inner` do outbox) — os três repositórios HTTP com Dio (`DioPrescriptionRepository`, `DioMedicationRepository`, `DioCheckoutRepository`), com classificação de falhas por `NetworkFailure` e chave de idempotência, existem e estão testados isoladamente, mas ainda não substituem os fakes na composição. O backend que faltava foi construído na Aula 33 (`functions/`, com os quatro endpoints e deduplicação por `Idempotency-Key`), então a razão original para mantê-los fora deixou de existir: falta apenas apontar `CheckoutApiClient` para a URL da function e trocar os repositórios na composição. Também não há cache nem retry automático por tipo de falha — isso e a troca dos repositórios fake pelos reais serão introduzidos em aulas posteriores.
